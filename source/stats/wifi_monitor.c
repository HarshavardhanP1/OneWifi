...
hash_map_t *get_wpa3_enhanced_sta_data_map(unsigned int vap_index) {
    ...
    pthread_mutex_unlock(&g_monitor_module.data_lock);
    wifi_util_info_print(WIFI_MON, "Retrieved STA data map for VAP index: %d vap name:%s \r\n", vap_index, vap_name);
...
int wpa3_enhanced_assoc_frame_data(frame_data_t *msg) {
    ...
    if (!isVapPrivate(msg->frame.ap_index)){
        wifi_util_info_print(WIFI_MON, "%s:%d It's not a private vap\r\n", __func__, __LINE__);
        return RETURN_OK;
    }
...
void wpa3_enhanced_connection_akms_count(telemetry_data_t *sta, int expected_akm, int actual_akm) {
    if (expected_akm == WPA3_SAE_EXT) {
        if (actual_akm == WPA3_SAE_EXT) {
            sta->expected_akm_24_24_count = sta->expected_akm_24_24_count + 1;
        } else if (actual_akm == WPA3_SAE) {
            sta->less_than_expected_akm_24_8_count = sta->less_than_expected_akm_24_8_count + 1;
        } else if (actual_akm == WPA2_PSK) {
            sta->less_than_expected_akm_24_2_count = sta->less_than_expected_akm_24_2_count + 1;
        }
    } else if (expected_akm == WPA3_SAE) {
        if (actual_akm == WPA3_SAE) {
            sta->expected_akm_8_8_count = sta->expected_akm_8_8_count + 1;
        } else if (actual_akm == WPA2_PSK) {
            sta->less_than_expected_akm_8_2_count = sta->less_than_expected_akm_8_2_count + 1;
        }
    } else if (expected_akm == WPA2_PSK) {
        if (actual_akm == WPA2_PSK) {
            sta->expected_akm_2_2_count = sta->expected_akm_2_2_count + 1;
        }
    }
}
...
void deinit_wifi_monitor()
{
    ...
    for (i = 0; i < getTotalNumberVAPs(); i++) {
        if(g_monitor_module.bssid_data[i].wpa3_sta_map != NULL) {
            ...
            hash_map_destroy(g_monitor_module.bssid_data[i].wpa3_st
